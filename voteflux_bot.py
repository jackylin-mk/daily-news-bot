...
def fetch_market_topics() -> str:
    ...
    try:
        part1 = _web_search(
            "請搜尋以下多個預測市場平台：Polymarket（polymarket.com）、Kalshi（kalshi.com）、"
            "Metaculus（metaculus.com）、Manifold Markets（manifold.markets）"
            "上與以下六個市場相關的預測題目：印度、孟加拉、越南、馬來西亞、菲律賓、泰國。"
            "每個國家盡量找 5~8 個真實題目原文（英文），涵蓋體育、財經、政治、娛樂等各種類型。"
            "只需列出題目原文與所屬國家及來源平台，不需要任何說明或分析。"
        )
        if part1:
...
# ─── 產生報告內容（JSON）─────────────────────────────────
# System prompt = 固定角色設定（適合 prompt cache）
# User prompt   = web search 結果 + 動態出題任務（每天不同，不快取）
...
5. **各市場熱門題目**：印度、孟加拉、越南、馬來西亞、菲律賓、泰國，各 3 題。
   ...
   【出題前必讀——合格題目公式】
   每道題必須符合此公式：「[具體主詞] 是否會在 [明確日期] 前 [具體動作 + 數字/官方名稱]？」
   
   【合格範例（格式參考，實際年份以當天日期為準）】
   ✅「印度央行是否會在下次例會上將基準利率調降至 5.75%？」
   ✅「孟加拉國家板球隊是否會在即將舉行的 ICC T20 世界盃小組賽中排名前兩名？」
   ✅「越南今年全年 FDI 流入額是否會突破 400 億美元？」
   ✅「馬來西亞 KLCI 指數是否會在本季結束前突破 1,700 點？」
   ✅「菲律賓下次全國選舉投票率是否會超過 80%？」
   ✅「泰國今年全年國際旅遊人次是否會突破 3,900 萬？」

   【絕對禁止清單——違反任何一條立即重出】
   ❌ 禁止捏造賽事：東南亞運動會（SEA Games）每兩年一屆，出題前必須先確認該屆賽事確實在指定時間舉辦，不確定就換財經或政治題
   ❌ 禁止「賽程安排」「場地準備」「名單確定」「報名程序」等行政事務——這些不是預測，是必然會發生的事
   ❌ 禁止模糊動詞：「放寬」「加強」「全面」「重大」「調整」「作出決策」→ 必須有具體數字或官方文件名稱
   ❌ 禁止「哪個/哪些/多少/什麼」開頭——永遠不是 Yes/No
   ❌ 禁止年份錯誤：現在是 {TODAY_STR}，所有日期必須在此之後
   ❌ 禁止「已知答案」的題目：若答案已可從新聞查到，不得出題
   ❌ 禁止捏造賽事：不確定賽事是否存在，就換一個財經或政治題目

   【類型多元要求】參考上方 Polymarket / Kalshi 真實熱門題目涵蓋的類型，讓 18 題自然分散在不同類型，避免全部集中在同一類（例如全是政治題）
