...
def generate_report_data() -> dict:
    user_prompt = (
        ...
        f"   步驟二：在候選池之外，主動找一個你認為「近期值得關注」的預測市場平台（不能是 Polymarket、Kalshi、VoteFlux、Hyperliquid、Predict.fun，也不能是候選池內的平台）。\n"
        ...
        f"2. **競品深度分析**\n"
        f"   平台：Polymarket, Kalshi, VoteFlux, Hyperliquid, Predict.fun, 加上 WEEKLY DISCOVERY 的平台（共 6 個）。\n"
        ...
        f"competitor_analysis 必須包含 6 個平台，scores/comments 的 key 必須與 analysis_dimensions 完全一致。"
    )
...
<p>正在跳轉到最新報告... <a href="voteflux-weekly-{NOW_FILE}.html">點此前往</a></p>
</body></html>""")
